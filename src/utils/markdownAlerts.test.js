...
> This is a note`
  ...
  test("renders WARNING alert as destructive", () => {
    const markdown = `> [!WARNING]
> This is a warning`

    const html = marked(markdown)
    expect(html).toContain('data-alert-type="warning"')
...
> Helpful advice here`
    ...
    const html = marked(markdown)
    expect(html).toContain('data-alert-type="tip"')
...
> Key information`
    ...
    const html = marked(markdown)
    expect(html).toContain('data-alert-type="important"')
    expect(html).toContain("Important")
    expect(html).toContain("Key information")
  })

  test("renders CAUTION alert as destructive", () => {
    const markdown = `> [!CAUTION]
> Be careful with this`

    const html = marked(markdown)
    expect(html).toContain('data-alert-type="caution"')
    expect(html).toContain("text-destructive")
...
> This is a tip alert.`
    ...
    const alertCount = (html.match(/data-slot="alert"/g) || []).length
